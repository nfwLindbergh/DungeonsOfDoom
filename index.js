...
window.onload = function() {
  class Game {
    ...
    beginFight() {
      ...
      this.monsterIds[2].value = this.monsterList[0].health;
    ...
    updatePlayerValues() {
      ...
      this.playerIds[0].value = this.player.name;
      this.playerIds[1].value = this.player.strength;
      this.playerIds[2].value = this.player.health;
    }
    ...
    upDateMonsterValue() {
      ...
      if (this.monsterList.length > 0) {
        this.monsterIds[0].value = this.monsterList[0].name;
        this.monsterIds[1].value = this.monsterList[0].strength;
        this.monsterIds[2].value = this.monsterList[0].health;
      } else {
  ...
  class InputReader {
    ...
    static readInAndCreatePlayer(game) {
      ...
      let playerInput = document.querySelector("#playerData");
      game.player = new Player(
        playerInput.dataset.name,
        playerInput.dataset.health,
        playerInput.dataset.strength
      );
    }

    static readInAndCreateMonster(game) {
      let monsters = document.querySelectorAll("#monsters > input");
      /**Vi använder en for-loop eftersom att det finns flera monster. Annars se ovan. */
      for (let monsterInput of monsters) {
        game.monsterList.push(
          new Monster(
            monsterInput.dataset.name,
            monsterInput.dataset.health,
            monsterInput.dataset.strength
          )
        );
      }
    }
  ...
  var fightButton = document.querySelector("#fight"); //Här deklarerar vi knappen "FIGHT!" som finns i .html för att kunna tilldela den en funktion.
  var fightLog = document.querySelector("#console");
